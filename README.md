...
- Dell CPi 노트북 디스크 이미지 (4.6GB, 5개 분할 파일)
- 용의자 계정: Mr. Evil
...
| 테스트 | 목적 |
|---|---|
| test_full_image_md5_matches_autopsy | 전체 이미지 MD5가 Autopsy 결과와 일치하는지 검증 |
| test_full_image_sha1_matches_autopsy | 전체 이미지 SHA1이 Autopsy 결과와 일치하는지 검증 |
| test_full_image_sha256_matches_autopsy | 전체 이미지 SHA256이 Autopsy 결과와 일치하는지 검증 |
| test_partial_image_differs_from_full | 분할 파일 일부만 계산 시 전체와 다른지 검증 |
| test_file_order_matters | 파일 순서가 바뀌면 해시값도 달라지는지 검증 |

## 검증 결과
| 알고리즘 | Autopsy | Python | 일치 여부 |
|---|---|---|---|
| MD5 | 4b206f73... | 4b206f73... | ✅ 일치 |
...
## GMDSOFT 연관성
본 프로젝트는 GMDSOFT MD-RED의 핵심 기능인
데이터 무결성 검증 프로세스를 QA 관점에서 구현했습니다.
